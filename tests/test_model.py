...
def test_guardrails_accept_clean_values_and_flag_safety_feature_violations() -> None:
    ...
    clean[:, MODEL_FEATURE_NAMES.index("max_object_displacement_m")] = [10.0, 20.0, 30.0]
    ...
    faulty = clean[:1].copy()
    faulty[0, MODEL_FEATURE_NAMES.index("max_object_displacement_m")] = 1_000.0
    scores, violations = score_guardrails(faulty, bounds)

    assert scores[0] > 0
    assert violations[0, GUARDRAIL_FEATURES.index("max_object_displacement_m")] > 0
...
def test_fast_path_excludes_trajectory_dependent_guardrails() -> None:
    ...
    scores, selected = select_guardrail_violations(
        violations, ("object_count_delta", "centroid_shift_m")
    )
